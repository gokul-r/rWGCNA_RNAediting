# Run WGCNA - standard pipeline
# Dataset: Not a standard gene expression dataset. This is actually RNA editing levels quantified in 6 human brain regions across prenatal and postnatal development. The final network is pretty noisy, but it serves as a illustrative example on how to run robust WGCNA.
# Gokul Ramaswami 8-19-2016

# set up R and load libraries
rm(list=ls());
options(stringsAsFactors = FALSE);
library(WGCNA);
enableWGCNAThreads(nThreads=12); # when requesting 12 CPUs
library(flashClust);
library(gplots)
